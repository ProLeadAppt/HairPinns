...
const ServiceDetailExperience = ({
  ...
  return (
    <main id="main-content" tabIndex={-1} data-service-detail="">
      ...
      <section data-service-detail-hero="" className="bg-[hsl(var(--hp-lavender))] text-[hsl(var(--hp-ink))]">
        <div className="mx-auto grid max-w-[78rem] gap-12 px-4 pb-16 pt-10 sm:px-6 sm:pb-20 sm:pt-14 lg:grid-cols-[minmax(0,0.68fr)_minmax(19rem,0.32fr)] lg:gap-20 lg:px-8 lg:pb-24 lg:pt-20">
          <div>
            <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.76)]">{categoryData.title}</p>
            <h1 className="mt-5 max-w-[16ch] font-heading text-[clamp(3rem,7vw,6.9rem)] font-semibold leading-[0.9] tracking-[-0.055em] text-[hsl(var(--hp-ink))]">
              {serviceData.title}
            </h1>
            {serviceData.tagline && (
            ...
            {serviceData.quickAnswer && (
              <p className="speakable-quick-answer mt-7 max-w-[45rem] text-base leading-7 text-[hsl(var(--hp-ink)/0.82)] sm:text-lg sm:leading-8">
                {serviceData.quickAnswer}
              </p>
            )}
          </div>

          <aside className="self-end border-t border-[hsl(var(--hp-ink)/0.3)] pt-6 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0" aria-label="Booking details">
            <dl className="grid grid-cols-2 gap-x-6 gap-y-7">
              <div>
                <dt className="font-mono text-[0.7rem] uppercase tracking-[0.15em] text-[hsl(var(--hp-ink)/0.7)]">Price</dt>
                <dd className="mt-2 font-heading text-3xl font-semibold text-[hsl(var(--hp-ink))]">{serviceData.price}</dd>
              </div>
              {serviceData.duration && (
                <div>
                  <dt className="font-mono text-[0.7rem] uppercase tracking-[0.15em] text-[hsl(var(--hp-ink)/0.7)]">Time</dt>
                  <dd className="mt-2 text-base font-semibold text-[hsl(var(--hp-ink))]">{serviceData.duration}</dd>
                </div>
              )}
            </dl>
            {venueReviewProof && (
            ...
            <p className="mt-4 text-xs leading-5 text-[hsl(var(--hp-ink)/0.64)]">Booking opens in Fresha. Choose the available time that suits you.</p>
          </aside>
        </div>
      </section>

      <section data-service-detail-overview="" className="bg-[hsl(var(--after-hours-cream))] text-[hsl(var(--hp-ink))]">
        <div className="mx-auto grid max-w-[78rem] gap-12 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-[0.36fr_0.64fr] lg:gap-20 lg:px-8 lg:py-24">
          <header>
            <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.74)]">01 / The appointment</p>
                ...
                <ol className="mt-6 space-y-4">
                  {serviceData.whatsIncluded.map((item, index) => (
                    <li key={item} className="grid grid-cols-[2rem_1fr] gap-3 border-t border-[hsl(var(--after-hours-plum)/0.16)] pt-4 text-sm leading-6">
                      <span className="font-mono text-[0.67rem] text-[hsl(var(--hp-ink)/0.72)]">{number(index)}</span>
                      <span>{item}</span>
                    </li>
                  ))}
                </ol>
      ...
      {serviceData.process.length > 0 && (
        <section data-service-detail-process="" className="bg-[hsl(var(--hp-lavender))] text-[hsl(var(--hp-ink))]">
          <div className="mx-auto max-w-[78rem] px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
            <div className="grid gap-8 border-b border-[hsl(var(--hp-ink)/0.24)] pb-10 lg:grid-cols-[0.36fr_0.64fr] lg:gap-20">
              <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.74)]">02 / In the chair</p>
              <h2 className="max-w-[13ch] font-heading text-[clamp(2.7rem,5vw,5.4rem)] font-semibold leading-[0.94] tracking-[-0.05em]" style={{ color: "hsl(var(--hp-ink))" }}>What to expect.</h2>
            </div>
            <ol className="grid md:grid-cols-3">
              {serviceData.process.map((step, index) => (
                <li key={step.step} className="border-b border-[hsl(var(--hp-ink)/0.2)] py-9 md:border-b-0 md:border-r md:px-8 md:py-12 first:md:pl-0 last:md:border-r-0 last:md:pr-0">
                  <p className="font-mono text-xs text-[hsl(var(--after-hours-copper))]">{number(index)}</p>
                  <h3 className="mt-7 max-w-[14ch] font-heading text-3xl font-semibold leading-[1.05]" style={{ color: "hsl(var(--hp-ink))" }}>{step.step}</h3>
                  <p className="mt-5 max-w-[30rem] text-sm leading-6 text-[hsl(var(--hp-ink)/0.76)]">{step.description}</p>
                </li>
              ))}
            </ol>
          </div>
      ...
      {serviceData.benefits.length > 0 && (
        <section data-service-detail-benefits="" className="bg-[hsl(var(--after-hours-cream))] text-[hsl(var(--hp-ink))]">
          <div className="mx-auto grid max-w-[78rem] gap-12 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-[0.36fr_0.64fr] lg:gap-20 lg:px-8 lg:py-24">
            <header>
              <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.74)]">03 / The result</p>
      ...
      {serviceData.homeCareBundles && (
        <section data-service-detail-homecare="" className="border-y border-[hsl(var(--after-hours-plum)/0.22)] bg-[hsl(var(--hp-lavender))] text-[hsl(var(--hp-ink))]">
          <div className="mx-auto grid max-w-[78rem] gap-12 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-[0.36fr_0.64fr] lg:gap-20 lg:px-8 lg:py-24">
            <header>
              <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.74)]">04 / Home care</p>
              ...
              {serviceData.homeCareBundles.intro && <p className="mt-6 max-w-sm text-sm leading-6 text-[hsl(var(--hp-ink)/0.76)]">{serviceData.homeCareBundles.intro}</p>}
            </header>
              ...
              {serviceData.homeCareBundles.products.map((product, index) => (
                <a key={product.link} href={product.link} className="group grid min-h-28 grid-cols-[2rem_1fr_auto] gap-3 border-b border-[hsl(var(--after-hours-plum)/0.22)] py-6" style={{ color: "hsl(var(--after-hours-plum))" }}>
                  ...
                  <span>
                    ...
                    <span className="mt-2 block max-w-[42rem] text-sm leading-6 text-[hsl(var(--hp-ink)/0.76)]">{product.reason}</span>
                  </span>
      ...
      {serviceData.faqs.length > 0 && (
        <section data-service-detail-faq="" className="bg-[hsl(var(--after-hours-cream))] text-[hsl(var(--hp-ink))]">
          <div className="mx-auto grid max-w-[78rem] gap-12 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-[0.36fr_0.64fr] lg:gap-20 lg:px-8 lg:py-24">
            <header>
              <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.74)]">05 / Before booking</p>
            ...
            <div className="border-t border-[hsl(var(--after-hours-plum)/0.3)]">
              {serviceData.faqs.map((faq, index) => (
                <details key={faq.question} className="group border-b border-[hsl(var(--after-hours-plum)/0.22)]">
                  ...
                  <p className="max-w-[44rem] pb-7 pl-11 text-sm leading-7 text-[hsl(var(--hp-ink)/0.78)]">{faq.answer}</p>
                </details>
              ))}
            </div>
          </div>
      ...
      {relatedServices.length > 0 && (
        <section data-service-detail-related="" className="bg-[hsl(var(--after-hours-cream))] text-[hsl(var(--hp-ink))]">
          <div className="mx-auto max-w-[78rem] px-4 pb-20 sm:px-6 lg:px-8 lg:pb-24">
            <div className="flex items-end justify-between gap-6 border-b border-[hsl(var(--after-hours-plum)/0.3)] pb-7">
              <div>
                <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.74)]">More appointments</p>
      ...
      <section data-service-detail-close="" className="bg-[hsl(var(--hp-lavender))] text-[hsl(var(--hp-ink))]">
        <div className="mx-auto grid max-w-[78rem] gap-10 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-[0.64fr_0.36fr] lg:gap-20 lg:px-8 lg:py-24">
          <div>
            <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.74)]">Ready when you are</p>
          ...
          <div className="self-end border-t border-[hsl(var(--hp-ink)/0.3)] pt-6">
            ...
            <a href={BUSINESS_NAP.phone.tel} className="mt-3 flex min-h-11 items-center border-b border-[hsl(var(--hp-ink)/0.26)] py-2 text-sm font-semibold" style={{ color: "hsl(var(--hp-ink))" }}>{BUSINESS_NAP.phone.display}</a>
            <div className="mt-6 flex flex-wrap gap-x-6 gap-y-3 text-sm text-[hsl(var(--hp-ink)/0.7)]">
              <address className="not-italic">{BUSINESS_NAP.address.full}</address>
              <Link to="/services" style={{ color: "hsl(var(--hp-ink))" }}>Back to the service menu →</Link>
            </div>
            <p className="mt-6 text-xs leading-5 text-[hsl(var(--hp-ink)/0.64)]">Price and duration are shown as listed in the Hair Pinns booking menu.</p>
          </div>
        </div>
      </section>
    </main>
