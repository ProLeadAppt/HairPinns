    ...
    <div className="grid gap-px bg-[hsl(var(--after-hours-plum)/0.24)] md:grid-cols-2">
      {[
        ...
        <div key={image.label} className="bg-[hsl(var(--after-hours-paper))]">
          ...
          <p className="border-t border-[hsl(var(--after-hours-plum)/0.2)] px-4 py-3 text-[0.62rem] font-semibold uppercase tracking-[0.15em] text-[hsl(var(--hp-ink)/0.72)]">{image.label}</p>
        </div>
      ))}
    </div>
    {caption ? <figcaption className="mt-4 text-sm leading-6 text-[hsl(var(--hp-ink)/0.72)]">{caption}</figcaption> : null}
  </figure>
);
