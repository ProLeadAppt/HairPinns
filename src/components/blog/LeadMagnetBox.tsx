    ...
    <aside className="my-16 bg-[hsl(var(--hp-lavender))] px-5 py-10 text-[hsl(var(--hp-ink))] sm:px-8 sm:py-12" aria-labelledby="guide-title">
      <div className="grid gap-8 md:grid-cols-[0.8fr_1.2fr] md:gap-12">
        <header>
          ...
          <p className="mt-5 text-sm leading-6 text-[hsl(var(--hp-ink)/0.72)]">Useful tips, honest product picks and 15% off your first online order. No daily inbox ambush.</p>
        </header>

        <form onSubmit={handleSubmit} className="border-t border-[hsl(var(--hp-ink)/0.26)] pt-2">
          <label className="block border-b border-[hsl(var(--hp-ink)/0.2)] py-4 text-[0.62rem] font-semibold uppercase tracking-[0.14em] text-[hsl(var(--hp-ink)/0.72)]">
            Email address
            <input
              type="email"
              value={email}
              ...
              className="mt-2 min-h-11 w-full border-0 bg-transparent p-0 text-base font-normal normal-case tracking-normal text-[hsl(var(--hp-ink))] placeholder:text-[hsl(var(--hp-ink)/0.72)] focus-visible:ring-0"
            />
          </label>
          <p className="py-5 text-xs leading-5 text-[hsl(var(--hp-ink)/0.72)]">
