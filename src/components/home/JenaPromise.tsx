...
const JenaPromise = () => {
  return (
    <section
      className="content-visibility-auto overflow-hidden bg-[hsl(var(--hp-lavender))] py-16 text-[hsl(var(--hp-ink))] sm:py-20 lg:py-28"
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-12 lg:items-end lg:gap-x-12 lg:gap-y-16">
          <header className="lg:col-span-7 lg:pb-4">
            ...
            <h2
              id="jena-proof-title"
              className="max-w-[12ch] font-heading text-[clamp(2rem,5vw,3.6rem)] font-normal leading-[1.08] tracking-[-0.05em] text-[hsl(var(--hp-ink))]"
            >
              A short shelf, chosen by a working hairdresser.
            </h2>
            <p className="mt-7 max-w-[42rem] text-base leading-7 text-[hsl(var(--hp-ink)/0.74)] sm:text-lg sm:leading-8">
          ...
          <figure className="lg:col-span-5">
            <div className="relative border border-[hsl(var(--hp-purple)/0.28)] p-2 sm:p-3">
              ...
              <span
                className="absolute right-0 top-0 bg-[hsl(var(--hp-lilac))] px-3 py-2 text-[0.58rem] font-semibold uppercase tracking-[0.16em] text-[hsl(var(--hp-ink))]"
                aria-hidden="true"
              >
                Founder / hairdresser
              </span>
            </div>
            <figcaption className="mt-4 flex items-center justify-between gap-4 border-b border-[hsl(var(--hp-purple)/0.22)] pb-4">
              <span className="font-heading text-xl text-[hsl(var(--hp-ink))]">Jena</span>
          ...
          <ol className="grid gap-8 border-t border-[hsl(var(--hp-purple)/0.22)] pt-8 sm:grid-cols-3 sm:gap-6 lg:col-span-12 lg:gap-10">
            {standards.map((standard) => (
              <li key={standard.index} className="grid grid-cols-[2.25rem_minmax(0,1fr)] gap-3 sm:block">
                ...
                <div className="sm:mt-5">
                  <h3 className="font-heading text-xl font-semibold leading-tight text-[hsl(var(--hp-ink))]">
                    {standard.title}
                  </h3>
                  <p className="mt-3 text-sm leading-6 text-[hsl(var(--hp-ink)/0.68)] sm:text-[0.95rem]">
                    {standard.body}
                  </p>
                </div>
              </li>
          ...
          <nav
            className="flex flex-col gap-5 border-t border-[hsl(var(--hp-purple)/0.22)] pt-8 sm:flex-row sm:items-center sm:gap-8 lg:col-span-12"
            aria-label="Jena’s shelf links"
          >
            <Link
              to="/collections"
              className="inline-flex min-h-12 items-center justify-between gap-6 bg-[hsl(var(--after-hours-cream))] px-5 text-sm font-semibold !text-[hsl(var(--hp-ink))] transition-colors hover:bg-[hsl(var(--hp-lilac))] sm:min-w-56"
            >
            ...
            <Link
              to="/about"
              className="inline-flex min-h-11 items-center gap-3 border-b border-[hsl(var(--hp-purple)/0.5)] text-sm font-semibold !text-[hsl(var(--hp-ink))] transition-colors hover:border-[hsl(var(--after-hours-copper))] hover:!text-[hsl(var(--after-hours-copper))] sm:min-h-0"
            >
