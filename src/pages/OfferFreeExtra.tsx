...
export default function OfferFreeExtra() {
  ...
  return (
    <div className="min-h-screen bg-[hsl(var(--after-hours-paper))] text-[hsl(var(--hp-ink))]">
      <SEOHead
      ...
      <main id="main-content" tabIndex={-1} data-free-extra-offer="">
        ...
        <section className="bg-[hsl(var(--hp-lavender))] text-[hsl(var(--hp-ink))]" aria-labelledby="free-extra-title">
          <div className="mx-auto grid max-w-7xl gap-12 px-4 py-14 sm:px-6 sm:py-20 lg:grid-cols-[minmax(0,1.15fr)_minmax(20rem,0.85fr)] lg:items-end lg:px-8 lg:py-24">
            <div>
              <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.88)]">
                {isActive ? "Now on / 05–12 August" : isUpcoming ? "Starts 9:00am AEST / 05 August" : "Offer ended / 12 August"}
              </p>
              <h1 id="free-extra-title" className="mt-6 max-w-[11ch] font-heading text-[clamp(3.4rem,8vw,7.5rem)] font-semibold leading-[0.88] tracking-[-0.055em] text-[hsl(var(--hp-ink))]">
                {promotion.headline}
              </h1>
              <p className="mt-8 max-w-2xl text-base leading-7 text-[hsl(var(--hp-ink)/0.8)] sm:text-lg sm:leading-8">
                {promotion.body}
              </p>
            </div>

            <div className="border-t border-[hsl(var(--hp-ink)/0.25)] pt-7 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0">
              <p className="font-heading text-2xl font-semibold">
                {isExpired ? "This offer has finished." : isUpcoming ? "Build your shortlist now." : "Three products in your bag. One is free."}
              </p>
              <p className="mt-4 text-sm leading-6 text-[hsl(var(--hp-ink)/0.72)]">
                {isExpired
              ...
              <Link
                ...
                className="mt-7 flex min-h-12 items-center justify-between bg-[hsl(var(--after-hours-cream))] px-5 py-3 text-sm font-semibold text-[hsl(var(--hp-ink))] transition-colors hover:bg-[hsl(var(--hp-lilac))] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--after-hours-copper))]"
              >
        ...
        {!isExpired && (
          <section className="border-b border-[hsl(var(--after-hours-plum)/0.18)]" aria-labelledby="claim-heading">
            <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
              <p className="after-hours-kicker text-[hsl(var(--hp-ink))]">How to claim it</p>
              ...
              <ol className="mt-12 grid border-t border-[hsl(var(--after-hours-plum)/0.22)] md:grid-cols-3">
                {steps.map((step) => (
                  <li key={step.number} className="border-b border-[hsl(var(--after-hours-plum)/0.22)] py-7 md:border-b-0 md:border-r md:px-7 md:first:pl-0 md:last:border-r-0 md:last:pr-0">
                    <p className="font-mono text-xs text-[hsl(var(--hp-ink))]">{step.number}</p>
                    <h3 className="mt-5 font-heading text-2xl font-semibold">{step.title}</h3>
                    <p className="mt-4 text-sm leading-6 text-[hsl(var(--hp-ink)/0.72)]">{step.body}</p>
                  </li>
                ))}
              </ol>
            </div>
          ...
          <section className="bg-[hsl(var(--after-hours-cream))]" aria-labelledby="gift-heading">
            <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
              <div className="flex flex-col gap-5 border-t border-[hsl(var(--after-hours-plum)/0.2)] pt-6 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  <p className="after-hours-kicker text-[hsl(var(--hp-ink))]">Choose one</p>
                  <h2 id="gift-heading" className="mt-5 font-heading text-4xl font-semibold sm:text-5xl">Pick the extra you’ll actually use.</h2>
                </div>
                <p className="max-w-md text-sm leading-6 text-[hsl(var(--hp-ink)/0.7)]">Gift colours and variants are subject to live stock. Add only one free extra per order.</p>
              </div>
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-mono text-xs text-[hsl(var(--hp-ink)/0.56)]">0{index + 1}</span>
                      </div>
                      <h3 className="mt-10 max-w-[14ch] font-heading text-3xl font-semibold leading-none">{gift.title}</h3>
                      <p className="mt-5 text-sm leading-6 text-[hsl(var(--hp-ink)/0.7)]">{giftNotes[gift.handle]}</p>
                    </div>
        ...
        <section className="border-y border-[hsl(var(--after-hours-plum)/0.2)]" aria-labelledby="terms-heading">
          <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
            <div className="grid gap-8 lg:grid-cols-[0.38fr_0.62fr]">
              <div>
                <p className="after-hours-kicker text-[hsl(var(--hp-ink))]">Offer terms</p>
                <h2 id="terms-heading" className="mt-5 font-heading text-3xl font-semibold">The useful details.</h2>
              </div>
              <div className="space-y-4 text-sm leading-6 text-[hsl(var(--hp-ink)/0.76)]">
                <p><strong className="text-[hsl(var(--hp-ink))]">Timing:</strong> Starts 9:00am AEST on 5 August 2026. Ends 9:00am AEST on 12 August 2026, or earlier if promotional stock or the 200-redemption allocation runs out.</p>
                <p><strong className="text-[hsl(var(--hp-ink))]">Eligibility:</strong> Buy two eligible full-size hair products in one order. Excludes QIQI products, sale items, bundles, gift cards, services and free-extra products.</p>
                <p><strong className="text-[hsl(var(--hp-ink))]">Gift:</strong> Add one listed free extra to the same bag. Limited to one free extra per order and limited to 200 redemptions. Colours and variants depend on live stock.</p>
                <p><strong className="text-[hsl(var(--hp-ink))]">Checkout:</strong> The gift discount appears automatically in Shopify checkout. This offer cannot be combined with another discount. Free standard shipping applies when the post-discount subtotal is $150 or more.</p>
                <p><strong className="text-[hsl(var(--hp-ink))]">Returns:</strong> If a qualifying product is returned, the free extra must also be returned unopened. If it is not returned, its retail value may be deducted from the refund.</p>
            ...
            <div className="mx-auto flex max-w-7xl flex-col gap-7 px-4 py-12 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm text-[hsl(var(--hp-ink)/0.7)]">Add all three to your bag, then check the discount in Shopify checkout.</p>
                </div>
              </div>
              <Link to={shopHref} className="flex min-h-12 min-w-[15rem] items-center justify-between bg-[hsl(var(--after-hours-cream))] px-5 py-3 text-sm font-semibold text-[hsl(var(--hp-ink))] hover:bg-[hsl(var(--hp-lilac))]">
