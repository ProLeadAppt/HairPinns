...
const DestinationCard = ({ destination, index }: { destination: CommerceDestination; index: number }) => (
  <li className="min-w-0">
    <Link
      to={destination.href}
      className="group block h-full border-t border-[hsl(var(--after-hours-plum)/0.24)] pt-3 text-[hsl(var(--hp-ink))] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--after-hours-copper))] focus-visible:ring-offset-4"
    >
      ...
      <div className="pt-4">
        ...
        <p className="mt-2 text-sm leading-6 text-[hsl(var(--hp-ink)/0.7)]">{destination.description}</p>
      </div>
    </Link>
  </li>
);
...
const Collections = () => {
  const { artwork, loading: artworkLoading } = useCollectionArtwork(SHOP_TAXONOMY.flatMap(group => group.destinations.map(destination => destination.handle)));
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main id="main-content" tabIndex={-1}>
              ...
              >
                ...
                <ul className="grid grid-cols-2 gap-x-4 gap-y-10 md:grid-cols-3 md:gap-x-7 lg:grid-cols-4">
                  {group.destinations.map((destination, index) => (
                    <DestinationCard key={destination.handle} destination={{ ...destination, image: artwork[destination.handle]?.url || '/placeholder.svg' }} index={index} />
                  ))}
                </ul>
                {group.secondaryDestinations?.length ? (
                  <div className="mt-12 border-t border-[hsl(var(--after-hours-plum)/0.2)] pt-6">
                    <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.72)]">More brands</p>
                    <ul className="mt-4 grid gap-0 border-t border-[hsl(var(--after-hours-plum)/0.16)] sm:grid-cols-3">
                      {group.secondaryDestinations.map((destination) => (
                        <li key={destination.handle} className="border-b border-[hsl(var(--after-hours-plum)/0.16)] sm:border-r sm:last:border-r-0">
                          <Link to={destination.href} className="flex min-h-12 items-center justify-between px-3 text-sm font-semibold text-[hsl(var(--hp-ink))] hover:text-[hsl(var(--after-hours-copper))] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[hsl(var(--after-hours-copper))]">
        ...
        <section className="border-y border-[hsl(var(--hp-lilac))] bg-[hsl(var(--hp-lavender))] py-10 text-[hsl(var(--hp-ink))] md:py-14" aria-labelledby="christmas-packs-heading">
          <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-8">
            <div>
              <p className="after-hours-kicker text-[hsl(var(--after-hours-copper))]">Seasonal edit / Christmas</p>
              <h2 id="christmas-packs-heading" className="mt-4 max-w-[16ch] font-heading text-3xl leading-tight text-[hsl(var(--hp-ink))] md:text-4xl">Christmas packs are here.</h2>
            ...
            <ol className="border-t border-[hsl(var(--hp-purple)/0.24)]">
              {CHRISTMAS_PRODUCTS.map((product, index) => (
                <li key={product.handle} className="border-b border-[hsl(var(--hp-purple)/0.24)]">
                  <Link to={product.href} className="group flex min-h-16 items-center gap-4 py-3 text-[hsl(var(--hp-ink))] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[hsl(var(--hp-purple))]">
        ...
        <section className="bg-[hsl(var(--after-hours-cream))] py-14 md:py-20">
          <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[minmax(0,1fr)_30rem] lg:items-end lg:gap-16 lg:px-8">
            <div>
              <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.72)]">Need a second opinion?</p>
            ...
            <div className="border-t border-[hsl(var(--after-hours-plum)/0.22)] pt-5">
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                ...
                <a href={BOOK_URL} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center text-sm font-medium text-[hsl(var(--hp-ink))] underline underline-offset-4">Or book direct</a>
              </div>
            </div>
          </div>
        </section>
