  ...
  return (
    <div className="min-h-screen bg-[hsl(var(--after-hours-cream))]">
      ...
      <main id="main-content" tabIndex={-1} data-booking-page="">
        ...
        <section data-booking-hero="" className="bg-[hsl(var(--hp-lavender))] text-[hsl(var(--hp-ink))]">
          <div className="mx-auto grid max-w-[78rem] gap-12 px-4 pb-16 pt-10 sm:px-6 sm:pb-20 sm:pt-14 lg:grid-cols-[minmax(0,0.67fr)_minmax(19rem,0.33fr)] lg:gap-20 lg:px-8 lg:pb-24 lg:pt-20">
            <div>
              <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.76)]">Hair Pinns / Bangor</p>
              <h1 className="mt-5 max-w-[12ch] font-heading text-[clamp(3.4rem,7vw,7.2rem)] font-semibold leading-[0.89] tracking-[-0.06em] text-[hsl(var(--hp-ink))]">
                Your appointment starts here.
              </h1>
              <p className="mt-8 max-w-[42rem] text-base leading-7 text-[hsl(var(--hp-ink)/0.82)] sm:text-lg sm:leading-8">
            ...
            <aside className="self-end border-t border-[hsl(var(--hp-ink)/0.3)] pt-7 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0" aria-label="Booking actions">
              <a
                href={BOOK_URL}
                target="_blank"
              ...
              <Link to="/services" className="mt-3 flex min-h-11 items-center justify-between border border-[hsl(var(--hp-ink)/0.34)] px-5 py-3 text-sm font-semibold" style={{ color: "hsl(var(--hp-ink))" }}>
                Browse the service menu
                <span aria-hidden="true">→</span>
              </Link>
              <p className="mt-5 text-xs leading-5 text-[hsl(var(--hp-ink)/0.66)]">Fresha opens in a new tab. No booking is made until you confirm there.</p>
            </aside>
          </div>
        </section>

        <section data-booking-steps="" className="bg-[hsl(var(--after-hours-cream))] text-[hsl(var(--hp-ink))]">
          <div className="mx-auto max-w-[78rem] px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
            <div className="grid gap-8 border-b border-[hsl(var(--after-hours-plum)/0.3)] pb-10 lg:grid-cols-[0.34fr_0.66fr] lg:gap-20">
              <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.74)]">01 / The handoff</p>
            ...
            <ol className="grid md:grid-cols-3">
              {bookingSteps.map((step, index) => (
                <li key={step.title} className="border-b border-[hsl(var(--after-hours-plum)/0.2)] py-9 md:min-h-64 md:border-b-0 md:border-r md:px-8 md:py-12 first:md:pl-0 last:md:border-r-0 last:md:pr-0">
                  <p className="font-mono text-xs text-[hsl(var(--hp-ink)/0.72)]">{String(index + 1).padStart(2, "0")}</p>
                  <h3 className="mt-7 max-w-[13ch] font-heading text-3xl font-semibold leading-[1.05]">{step.title}</h3>
                  <p className="mt-5 max-w-[26rem] text-sm leading-6 text-[hsl(var(--hp-ink)/0.76)]">{step.text}</p>
                </li>
              ))}
            </ol>
          </div>
        ...
        <section data-booking-notes="" className="border-y border-[hsl(var(--after-hours-plum)/0.22)] bg-[hsl(var(--hp-lavender))] text-[hsl(var(--hp-ink))]">
          <div className="mx-auto grid max-w-[78rem] gap-12 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-[0.34fr_0.66fr] lg:gap-20 lg:px-8 lg:py-24">
            <header>
              <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.74)]">02 / Before you confirm</p>
            ...
            <ol className="border-t border-[hsl(var(--after-hours-plum)/0.3)]">
              {bookingNotes.map(([term, description], index) => (
                <li key={term} className="grid grid-cols-[2rem_1fr] gap-3 border-b border-[hsl(var(--after-hours-plum)/0.22)] py-6 sm:grid-cols-[2.5rem_12rem_1fr] sm:gap-5">
                  <span className="font-mono text-[0.67rem] text-[hsl(var(--hp-ink)/0.72)]">{String(index + 1).padStart(2, "0")}</span>
                  <h3 className="font-heading text-xl font-semibold leading-tight">{term}</h3>
                  <p className="col-start-2 mt-2 text-sm leading-6 text-[hsl(var(--hp-ink)/0.78)] sm:col-start-3 sm:mt-0">{description}</p>
                </li>
              ))}
            </ol>
          </div>
        ...
        <section data-booking-faq="" className="bg-[hsl(var(--after-hours-cream))] text-[hsl(var(--hp-ink))]">
          <div className="mx-auto grid max-w-[78rem] gap-12 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-[0.34fr_0.66fr] lg:gap-20 lg:px-8 lg:py-24">
            <header>
              <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.74)]">03 / Questions</p>
        ...
        <section data-booking-close="" className="bg-[hsl(var(--hp-lavender))] text-[hsl(var(--hp-ink))]">
          <div className="mx-auto grid max-w-[78rem] gap-10 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-[0.64fr_0.36fr] lg:gap-20 lg:px-8 lg:py-24">
            <div>
              ...
              <h2 className="mt-5 max-w-[12ch] font-heading text-[clamp(3rem,6vw,6rem)] font-semibold leading-[0.92] tracking-[-0.05em]" style={{ color: "hsl(var(--hp-ink))" }}>See what times are available.</h2>
            </div>
            <div className="self-end border-t border-[hsl(var(--hp-ink)/0.3)] pt-6">
              ...
              <a href="https://wa.me/61416037663?text=Hi%20Jena%2C%20I%27d%20like%20help%20choosing%20a%20service" target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center border-b border-[hsl(var(--hp-ink)/0.26)] py-2 text-sm" style={{ color: "hsl(var(--hp-ink))" }}>WhatsApp Jena ↗</a>
              <div className="mt-6 flex flex-wrap gap-5 text-xs text-[hsl(var(--hp-ink)/0.7)]">
                <Link to="/privacy" style={{ color: "hsl(var(--hp-ink))" }}>Privacy</Link>
                <Link to="/terms" style={{ color: "hsl(var(--hp-ink))" }}>Terms</Link>
              </div>
            </div>
          </div>
        </section>
