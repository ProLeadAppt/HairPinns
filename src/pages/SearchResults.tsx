  ...
  return (
    <div data-search-page="" className="min-h-screen bg-[hsl(var(--after-hours-cream))]">
      ...
      <main id="main-content" tabIndex={-1}>
        <section data-search-hero="" className="border-b border-[hsl(var(--hp-ink)/0.14)] bg-[hsl(var(--hp-lavender))] text-[hsl(var(--hp-ink))]">
          <div className="mx-auto max-w-7xl px-5 pb-14 pt-5 sm:px-8 lg:px-12 lg:pb-20">
            ...
            <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-end">
              <div>
                ...
                <h1 className="mt-5 max-w-[13ch] font-heading text-[clamp(3rem,8vw,7rem)] font-semibold leading-[0.88] tracking-[-0.055em] text-[hsl(var(--hp-ink))]">
                  {query ? <>Results for <em className="font-normal">“{query}”</em></> : <>What are you <em className="font-normal">looking for?</em></>}
                </h1>
              </div>
              <p className="max-w-sm border-l border-[hsl(var(--after-hours-copper))] pl-5 text-sm leading-6 text-[hsl(var(--hp-ink)/0.76)]">
        ...
        <section data-search-desk="" className="border-b border-[hsl(var(--after-hours-plum)/0.2)]">
          <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:px-12">
            <form role="search" onSubmit={submitSearch} className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
              ...
              <input id="search-page-query" type="search" value={draftQuery} onChange={(event) => setDraftQuery(event.target.value)} placeholder="Product, brand, concern or guide" className="min-h-12 w-full border border-[hsl(var(--after-hours-plum)/0.35)] bg-transparent px-4 py-3 text-[hsl(var(--hp-ink))] outline-none placeholder:text-[hsl(var(--hp-ink)/0.52)] focus:border-[hsl(var(--after-hours-plum))] focus:ring-2 focus:ring-[hsl(var(--after-hours-copper)/0.42)]" />
        ...
        <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 lg:px-12 lg:py-20">
          {loading ? (
            <section data-search-page-loading="" aria-label="Loading search results" aria-busy="true">
              <p className="after-hours-kicker text-[hsl(var(--hp-ink)/0.62)]">Searching the shelf</p>
            ...
            <section data-search-start="" className="grid gap-10 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
              <div>
                <p className="after-hours-kicker text-brand-500">A useful place to start</p>
                <h2 className="mt-5 max-w-[11ch] font-heading text-4xl font-semibold leading-[0.98] text-[hsl(var(--hp-ink))] sm:text-6xl">Search by what your hair needs.</h2>
              </div>
              <div className="border-t border-[hsl(var(--after-hours-plum)/0.3)]">
                {POPULAR_SEARCHES.map((term, index) => (
                  <Link key={term} to={`/search?q=${encodeURIComponent(term)}`} className="grid min-h-16 grid-cols-[2.5rem_1fr_auto] items-center border-b border-[hsl(var(--after-hours-plum)/0.22)] py-3 text-[hsl(var(--hp-ink))]" style={{ color: "hsl(var(--after-hours-plum))" }}>
                    <span className="font-mono text-[0.62rem] text-[hsl(var(--hp-ink)/0.78)]">{String(index + 1).padStart(2, "0")}</span><span className="font-heading text-xl font-semibold">{term}</span><span aria-hidden="true">→</span>
                  </Link>
                ))}
              </div>
            </section>
          ) : productError && articles.length === 0 ? (
            <section data-search-page-error="" className="border-y border-[hsl(var(--after-hours-plum)/0.3)] py-10 text-[hsl(var(--hp-ink))]">
              <p className="after-hours-kicker text-brand-500">Search interrupted</p>
              <h2 className="mt-4 font-heading text-4xl font-semibold">Products could not load.</h2>
              <p className="mt-4 max-w-xl text-sm leading-6 text-[hsl(var(--hp-ink)/0.72)]">Try the search again, browse the full product catalogue, or read Jena’s guides.</p>
            ...
            <section data-search-page-empty="" className="grid gap-8 border-y border-[hsl(var(--after-hours-plum)/0.3)] py-10 text-[hsl(var(--hp-ink))] lg:grid-cols-[minmax(0,1fr)_20rem]">
              <div><p className="after-hours-kicker text-brand-500">No exact match</p><h2 className="mt-4 max-w-[14ch] font-heading text-4xl font-semibold leading-tight">Nothing found for “{query}”.</h2><p className="mt-4 max-w-xl text-sm leading-6 text-[hsl(var(--hp-ink)/0.72)]">Check the spelling, try a broader hair concern, or browse all products.</p></div>
            ...
            <div data-search-page-results="" className="space-y-20 text-[hsl(var(--hp-ink))]">
              <div className="flex flex-wrap items-end justify-between gap-5 border-b border-[hsl(var(--after-hours-plum)/0.28)] pb-5">
                <div><p className="after-hours-kicker text-brand-500">Search return / {String(totalResults).padStart(2, "0")}</p><p className="mt-2 text-sm text-[hsl(var(--hp-ink)/0.68)]">{products.length} {products.length === 1 ? "product" : "products"} · {articles.length} {articles.length === 1 ? "guide" : "guides"}</p></div>
              ...
              {products.length > 0 && (
                <section data-search-product-results="" aria-labelledby="product-results-heading">
                  <div className="mb-8 flex items-end justify-between border-b border-[hsl(var(--after-hours-plum)/0.24)] pb-4"><div><p className="after-hours-kicker text-brand-500">01 / Shop</p><h2 id="product-results-heading" className="mt-3 font-heading text-4xl font-semibold">Products</h2></div><Link to="/collections" className="hidden min-h-11 items-center text-xs font-semibold underline underline-offset-4 sm:inline-flex" style={{ color: "hsl(var(--after-hours-plum))" }}>All products</Link></div>
              ...
              {articles.length > 0 && (
                <section data-search-article-results="" aria-labelledby="article-results-heading">
                  <div className="mb-8 border-b border-[hsl(var(--after-hours-plum)/0.24)] pb-4"><p className="after-hours-kicker text-brand-500">02 / Learn</p><h2 id="article-results-heading" className="mt-3 font-heading text-4xl font-semibold">Jena’s guides</h2></div>
                  <div className="border-t border-[hsl(var(--after-hours-plum)/0.24)]">
                    {articles.map((article, index) => <article key={article.slug} className="grid gap-4 border-b border-[hsl(var(--after-hours-plum)/0.22)] py-5 sm:grid-cols-[7rem_minmax(0,1fr)_auto] sm:items-center"><Link to={article.url} className="aspect-[4/3] overflow-hidden bg-[hsl(var(--after-hours-plum)/0.08)]"><img src={article.image} alt={article.title} width="280" height="210" loading="lazy" decoding="async" className="h-full w-full object-cover" /></Link><div><p className="font-mono text-[0.6rem] text-[hsl(var(--hp-ink)/0.55)]">{String(index + 1).padStart(2, "0")} / {article.category}</p><h3 className="mt-2 font-heading text-xl font-semibold leading-tight"><Link to={article.url} style={{ color: "hsl(var(--after-hours-plum))" }}>{article.title}</Link></h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-[hsl(var(--hp-ink)/0.68)]">{article.excerpt}</p></div><Link to={article.url} className="inline-flex min-h-11 items-center text-xs font-semibold underline underline-offset-4" style={{ color: "hsl(var(--after-hours-plum))" }}>Read guide</Link></article>)}
                  </div>
                </section>
              )}
            </div>
